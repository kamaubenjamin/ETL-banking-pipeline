...
class ETLPipeline:
    ...
    def run(self, source_type, uploaded_df=None, mode=None, selector=None,
        ...
        pipeline_name = f"etl_{source_type}".replace(" ", "_").lower()
        pipeline_logger = PipelineLogger(pipeline_name)
        pipeline_logger.start(
            metadata={
                ...
                # Airflow DAGs can pass dag_id/task_id/run_id here later.
                "orchestrator": "ETLPipeline.run",
            }
        )
        ...
        try:
            ...
            pipeline_logger.log_ingestion_batch(
                ...
                metadata={
                    "stage": "extract",
                    # Kafka producers can use this event boundary for
                    # source-level ingestion topics later.
                    "batch_boundary": "source_extract",
                },
            )
            ...
            elif result.get("extract", {}).get("rows"):
                rows_processed = result["extract"]["rows"]
            self.audit.log_error("pipeline_failure", e, {"source_type": source_type})
